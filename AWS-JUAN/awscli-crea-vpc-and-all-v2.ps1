#=================
# Paco Cuadrado fcuadradoa01@educantabria.es (Juan Carlos González Aguilera)
# 2025-03-07
...
$region = "us-east-1"
#
$bloque_cidr_vpc = "10.115.0.0/16"
$bloque_subred_publica = "10.115.0.0/25"
$bloque_subred_privada = "10.115.0.128/25"
...
$vpcId = (aws ec2 create-vpc `
    ...
    --region $region `
    --tag-specifications 'ResourceType=vpc,Tags=[{Key=Name,Value=JuanNextCloud}]' `
...
$publicSubnetId = (aws ec2 create-subnet `
    ...
    --tag-specifications 'ResourceType=subnet,Tags=[{Key=Name,Value=JuanNextCloud-publica}]' `
...
$privateSubnetId = (aws ec2 create-subnet `
    ...
    --region $region --availability-zone "us-east-1a" `
    --tag-specifications 'ResourceType=subnet,Tags=[{Key=Name,Value=JuanNextCloud-privada}]' `
    --query 'Subnet.SubnetId' --output text)
